...
except ImportError:  # pragma: no cover - dependency not available in CI environment
    pytest.skip(
        "PyQt5 (Qt bindings) not available, skipping tests that require Qt",
...
PROMPT_MSG = (
    "Asking question <qutebrowser.utils.usertypes.Question "
...
def wait_for_download_prompt(tmpdir, quteproc, path):
    ...
    quteproc.wait_for(
        message="Entering mode KeyMode.prompt " "(reason: question asked)"
    )
...
@bdd.then(
    bdd.parsers.parse("The downloaded file {filename} should be " "{size} bytes big")
)
