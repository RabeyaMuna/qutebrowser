...
class TestQuteLastPassComponents:
    ...
    def test_pass_subprocess_args(self, subprocess_mock):
        ...
        subprocess_mock.assert_called_once_with(
            ["lpass", "show", "-x", "-j", "-G", "\\bexample\\.com"], capture_output=True
        )
    ...
    def test_pass_returns_error(self, subprocess_mock):
        ...
        error_message = (
            "Error: Could not find decryption key. "
...
class TestQuteLastPassMain:
    ...
    def test_main_no_candidates(
        self, subprocess_mock, arguments_mock, stderr_mock, qutecommand_mock
    ):
    ...
    def test_main_lpass_failure(
        self, subprocess_mock, arguments_mock, stderr_mock, qutecommand_mock
    ):
        """Test correct exit code and message on lpass failure."""
        error_message = (
            b"Error: Could not find decryption key. "
    ...
    def test_main_username_only_flag(
        self, subprocess_mock, arguments_mock, qutecommand_mock
    ):
    ...
    def test_main_password_only_flag(
        self, subprocess_mock, arguments_mock, qutecommand_mock
    ):
        ...
        qutecommand_mock.assert_has_calls(
            [call("fake-key \\f\\o\\o\\b\\a\\r"), call("mode-enter insert")]
        )

    def test_main_multiple_candidates(
        self, subprocess_mock, arguments_mock, qutecommand_mock
    ):
        ...
        subprocess_mock.assert_has_calls(
            [
                call(
                    ["lpass", "show", "-x", "-j", "-G", "\\bwww\\.example\\.com"],
                    capture_output=True,
                ),
                call(
                    ["rofi", "-dmenu"],
    ...
    def test_main_merge_candidates(
        self, subprocess_mock, arguments_mock, qutecommand_mock
    ):
        ...
        subprocess_mock.assert_has_calls(
            [
                ...
                call(
                    ["lpass", "show", "-x", "-j", "-G", "\\bwwwexample"],
                    capture_output=True,
                ),
                call(
                    ["lpass", "show", "-x", "-j", "-G", "\\bexample"],
                    capture_output=True,
                ),
                call(
                    ["rofi", "-dmenu"],
