...
class TestCommandLineEdit:
    ...
    def test_selection_cursor_left(self, qtbot, cmd_edit):
        ...
        for _ in ":hello":
            qtbot.keyClick(
                cmd_edit, Qt.Key.Key_Left, modifier=Qt.KeyboardModifier.ShiftModifier
            )
        assert cmd_edit.cursorPosition() == len(":")
        assert cmd_edit.selectionStart() == len(":")
...
class TestFullscreenNotification:
    ...
    @pytest.mark.parametrize(
        "bindings, text",
        [
            ({"<escape>": "fullscreen --leave"}, "Press <Escape> to exit fullscreen."),
...
class TestInspectorSplitter:
    ...
    @pytest.mark.parametrize(
        "old_window_size, preferred_size, new_window_size, exp_inspector_size",
        [
            ...
            (
                ...
                200,
                100,
            ),  # 300px of 600px -> 100px of 200px (1/2)
        ],
    )
    @pytest.mark.parametrize(
        "position",
