...
def test_word_wrap(view, qtbot):
    ...
    text = (
        "Athene, the bright-eyed goddess, answered him at once: Father of "
...
def test_rich_text(view, qtbot, rich, higher, expected_format, replace):
    ...
    ctx = (
        qtbot.wait_signal(view._clear_timer.timeout)
        if replace is None
...
def test_show_multiple_messages_longer(view, count, expected):
    ...
    for message_number in range(1, count + 1):
        view.show_message(
            message.MessageInfo(usertypes.MessageLevel.info, f"test {message_number}")
        )
    assert view._clear_timer.interval() == expected


@pytest.mark.parametrize(
    "replace1, replace2, length",
    [
...
def test_replaced_messages(view, replace1, replace2, length):
    ...
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test", replace=replace1)
    )
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test 2", replace=replace2)
    )
...
def test_replacing_different_severity(view):
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test", replace="testid")
    )
...
def test_replacing_changed_text(view):
    ...
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test 3", replace="testid")
    )
...
def test_replacing_geometry(qtbot, view):
    ...
    with qtbot.wait_signal(view.update_geometry):
        view.show_message(
            message.MessageInfo(usertypes.MessageLevel.info, "test 2", replace="testid")
        )
...
def test_click_messages(qtbot, view, button, count):
    ...
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test mouse click")
    )
    view.show_message(
        message.MessageInfo(usertypes.MessageLevel.info, "test mouse click 2")
    )
