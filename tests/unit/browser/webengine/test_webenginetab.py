...
class ScriptsHelper:
    ...
    def get_scripts(self, prefix="GM-"):
        return [
            s
            for s in self._tab._widget.page().scripts().toList()
...
class TestWebengineScripts:
    ...
    def test_greasemonkey_document_end_workaround(self, monkeypatch, scripts_helper):
        ...
        monkeypatch.setattr(
            greasemonkey.objects, "backend", usertypes.Backend.QtWebEngine
        )
    ...
    def test_greasemonkey_duplicate_name(
        self, scripts_helper, header1, header2, expected_names
    ):
        template = """
            // ==UserScript==
            {header}
...
class TestFindFlags:
    @pytest.mark.parametrize(
        "case_sensitive, backward, expected",
        [
            (
                True,
                True,
