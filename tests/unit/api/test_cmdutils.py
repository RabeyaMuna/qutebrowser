...
class TestCheckOverflow:
    ...
    def test_bad(self):
        ...
        with pytest.raises(
            ...
            match="Numeric argument is too large for internal int representation.",
        ):
...
class TestCheckExclusive:
    ...
    def test_bad(self):
        with pytest.raises(
            cmdutils.CommandError, match="Only one of -x/-y/-z can be given!"
        ):
...
class TestRegister:
    ...
    @pytest.mark.parametrize(
        "inp, expected", [(["--arg"], True), (["-a"], True), ([], False)]
    )
    ...
    def test_self_without_instance(self):
        with pytest.raises(
            TypeError, match="fun is a class method, but instance was not given!"
        ):
    ...
    def test_instance_without_self(self):
        with pytest.raises(
            TypeError, match="fun is not a class method, but instance was given!"
        ):
    ...
    def test_var_kw(self):
        with pytest.raises(
            TypeError, match="fun: functions with varkw arguments are not supported!"
        ):
    ...
    def test_fill_invalid(self):
        with pytest.raises(
            TypeError, match="fun: Invalid value='foo' for argument 'arg'!"
        ):
    ...
    def test_count_without_default(self):
        with pytest.raises(
            TypeError, match="fun: handler has count parameter without default!"
        ):
    ...
    @pytest.mark.parametrize(
        "annotation, inp, choices, expected",
        [
            ("int", "42", None, 42),
    ...
    def test_keyword_only_without_default(self):
        ...
        with pytest.raises(
            TypeError,
            match="fun: handler has keyword only argument 'target' without default!",
        ):
...
class TestArgument:
    ...
    def test_wrong_order(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"@cmdutils.argument got called "
...
class TestRun:
    ...
    def test_instance_unavailable_with_backend(self, monkeypatch):
        ...
        @cmdutils.register(
            instance="doesnotexist", backend=usertypes.Backend.QtWebEngine
        )
        def fun(self):
            """Blah."""
